...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  // NOTE: this MUST run before the isChatInputCommand() guard below, otherwise

  // the quest dropdown never updates the embed.

  if (interaction.isStringSelectMenu && interaction.isStringSelectMenu()) {

    if (interaction.customId && interaction.customId.startsWith('quest_menu_v2')) {

      const questMod = client.modules.get('q');

      if (questMod && typeof questMod.handleSelectMenu === 'function') {

        try {

          await questMod.handleSelectMenu(interaction);

        } catch (err) {

          console.error('[INTERACTION ERROR] quest_menu_v2 failure:', err);
